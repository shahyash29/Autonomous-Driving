...
class ENet_model(object):
    ...
    def __init__(self, model_id, img_height=512, img_width=1024, batch_size=4):
        ...
        self.class_weights = cPickle.load(open(self .project_dir+ "data/class_weights.pkl", "rb"))
    ...
    def add_placeholders(self):
        ...
        self.onehot_labels_ph = tf.compat.v1.placeholder(tf.float32,
        ...
        self.early_drop_prob_ph = tf.compat.v1.placeholder(tf.float32, name="early_drop_prob_ph")

        # dropout probability in the later layers of the network:
        self.late_drop_prob_ph = tf.compat.v1.placeholder(tf.float32, name="late_drop_prob_ph")
    ...
    def add_logits(self):
        ...
        # fullconv:
        network = slim.conv2d_transpose(network, self.no_of_classes,
    ...
    def add_loss_op(self):
        ...
        seg_loss_per_pixel = tf.compat.v1.losses.softmax_cross_entropy(
        ...
        self.loss = (self.seg_loss +
                    tf.add_n(tf.compat.v1.get_collection("encoder_wd_losses")) +
                    tf.add_n(tf.compat.v1.get_collection("decoder_wd_losses")))
    ...
    def initial_block(self, x, scope):
        ...
        b_conv = self.get_variable_weight_decay(scope + "/b", shape=[13], # ([out_depth])
                    ...
                    loss_category="encoder_wd_losses")
        conv_branch = tf.nn.conv2d(x, filters=W_conv, strides=[1, 2, 2, 1],
                    padding="SAME") + b_conv

        # max pooling branch:
        pool_branch = tf.nn.max_pool2d(input=x, ksize=[1, 2, 2, 1],
    ...
    def encoder_bottleneck_regular(self, x, output_depth, drop_prob, scope,
        ...
        if downsampling:
            W_conv = self.get_variable_weight_decay(scope + "/W_proj",
                        ...
                        initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"),
                        loss_category="encoder_wd_losses")
            conv_branch = tf.nn.conv2d(conv_branch, filters=W_conv, strides=[1, 2, 2, 1],
            ...
            W_proj = self.get_variable_weight_decay(scope + "/W_proj",
                        ...
                        initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"),
                        loss_category="encoder_wd_losses")
            conv_branch = tf.nn.conv2d(conv_branch, filters=W_proj, strides=[1, 1, 1, 1],
                        padding="VALID") # NOTE! no bias terms
        # # # batch norm and PReLU:
        conv_branch = slim.batch_norm(conv_branch)
        ...
        b_conv = self.get_variable_weight_decay(scope + "/b_conv", shape=[internal_depth], # ([out_depth])
                    initializer=tf.compat.v1.constant_initializer(0),
                    loss_category="encoder_wd_losses")
        conv_branch = tf.nn.conv2d(conv_branch, filters=W_conv, strides=[1, 1, 1, 1],
                    padding="SAME") + b_conv
        # # # batch norm and PReLU:
        conv_branch = slim.batch_norm(conv_branch)
        ...
        W_exp = self.get_variable_weight_decay(scope + "/W_exp",
                    ...
                    initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"),
                    loss_category="encoder_wd_losses")
        conv_branch = tf.nn.conv2d(conv_branch, filters=W_exp, strides=[1, 1, 1, 1],
                    padding="VALID") # NOTE! no bias terms
        # # # batch norm:
        conv_branch = slim.batch_norm(conv_branch)
        # NOTE! no PReLU here
    ...
    def encoder_bottleneck_asymmetric(self, x, output_depth, drop_prob, scope, proj_ratio=4):
        ...
        W_conv1 = self.get_variable_weight_decay(scope + "/W_conv1",
                    ...
                    initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"),
                    loss_category="encoder_wd_losses")
        conv_branch = tf.nn.conv2d(conv_branch, filters=W_conv1, strides=[1, 1, 1, 1],
        ...
        b_conv2 = self.get_variable_weight_decay(scope + "/b_conv2", shape=[internal_depth], # ([out_depth])
                    initializer=tf.compat.v1.constant_initializer(0),
                    loss_category="encoder_wd_losses")
        conv_branch = tf.nn.conv2d(conv_branch, filters=W_conv2, strides=[1, 1, 1, 1],
    ...
    def decoder_bottleneck(self, x, output_depth, scope, proj_ratio=4,
        ...
        if upsampling:
            ...
            main_branch = tf.nn.conv2d(main_branch, filters=W_upsample, strides=[1, 1, 1, 1],
                        padding="VALID") # NOTE! no bias terms
            # # # batch norm:
            main_branch = slim.batch_norm(main_branch)
            # NOTE! no ReLU here
    ...
    def get_variable_weight_decay(self, name, shape, initializer, loss_category,
                dtype=tf.float32):
        variable = tf.compat.v1.get_variable(name, shape=shape, dtype=dtype,
                    initializer=initializer)

        # add a variable weight decay loss:
        weight_decay = self.wd*tf.nn.l2_loss(variable)
        tf.compat.v1.add_to_collection(loss_category, weight_decay)
